...
@Component
@StepScope
public class EmailStepTwoTasklet implements Tasklet {
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunk) throws Exception {
		try {
			String nombreTemplate = ConfrontaConstants.STEP_TWO_TEMPLATE;
			emailService.sendEmailStepTwo(nombreTemplate);
		} catch(Exception exc) {
			exc.printStackTrace();
		}
